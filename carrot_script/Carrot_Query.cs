...
namespace Carrot
{
            ...
            if (this.doc["fields"] != null)
            {
                ...
                foreach (var key in fields.Keys)
                {
                    ...
                    if (val["arrayValue"] != null)
                    {
                        ...
                        IList arrayValue = values["values"] as IList;
                        if (arrayValue == null) arrayValue = (IList)Json.Deserialize("[]");
    ...
    public class StructuredQuery
    {
        ...
        {
            this.s_where = ",\"where\":{\"fieldFilter\":{\"field\":{\"fieldPath\":\"" + key + "\"},\"op\":\"" + op + "\",\"value\":{\"stringValue\":\"" + val + "\"}}}";
        }

        public void Add_where(string key, Query_OP op = Query_OP.EQUAL, string val = "")
        {
            this.list_where.Add("{\"fieldFilter\":{\"field\":{\"fieldPath\":\"" + key + "\"},\"op\":\"" + op + "\",\"value\":{\"stringValue\":\"" + val + "\"}}}");
        }

        public void Add_order(string s_field_orderby, Query_Order_Direction direction = Query_Order_Direction.ASCENDING)
        {
            this.s_order = ",\"orderBy\":[{\"field\":{\"fieldPath\":\"" + s_field_orderby + "\"},\"direction\":\"" + direction + "\"}]";
        }
        ...
        public string ToJson()
        {
            ...
            else if (this.list_where.Count > 0)
            {
